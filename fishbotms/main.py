...
    kb.row(
        types.InlineKeyboardButton(text="🎣 Закинуть", callback_data="throw"),
        types.InlineKeyboardButton(text="🎒 Инвент", callback_data="inv")
    )
    kb.row(
        types.InlineKeyboardButton(text="🗺️ Локации", callback_data="loc"),
        types.InlineKeyboardButton(text="🧪 Наживка", callback_data="bait_menu")
    )
    kb.row(
        types.InlineKeyboardButton(text="📖 Альманах", callback_data="almanac"),
        # Сетку сделаем кнопкой, но она всё равно будет вызывать функцию use_grid
        types.InlineKeyboardButton(text="🕸 Сетка", callback_data="grid_call")
    )
    kb.row(types.InlineKeyboardButton(text="🏆 Топ", callback_data="top"))
    return kb.as_markup()нич
...
    # Чистим название от " x1" или " (10.5 💰)"
    fish_name = raw_text.split(" x")[0].split(" (")[0].strip()
    sid, rid = msg.from_user.id, msg.reply_to_message.from_user.id

    with db.connection:
        # Ищем рыбу без учета регистра (NOCASE)
...
    
    f_key = random.choice(list(FISH_DATA.keys())) # Для теста берем совсем любую
